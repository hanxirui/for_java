...
public class BeanCellMapping {
	...
	private String extractBeanName(String fullPropertyName) {
		...
		if (dotIndex < 0) {
			throw new IllegalArgumentException(
					"Full property name must contain period. Can't extract bean name from " + fullPropertyName);
		} else {
	...
	public void populateBean(String dataString, Map beans)
		...
		if (beans.containsKey(beanKey)) {
			...
			// Object value = null;
			// if (!(nullAllowed && dataString == null)) { // set only if null
			// is not allowed!
			// value = ConvertUtils.convert(dataString, dataType);
			// }

			PropertyUtils.setProperty(bean, propertyName, dataString);
		} else {
	...
	public Class getPropertyType(Map beans)
