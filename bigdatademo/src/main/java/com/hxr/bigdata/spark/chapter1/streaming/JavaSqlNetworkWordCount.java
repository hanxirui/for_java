 ...
 * Usage: JavaSqlNetworkWordCount <hostname> <port> <hostname> and <port> describe the TCP server that Spark Streaming
 * would connect to receive data. To run this on your local machine, you need to first run a Netcat server `$ nc -lk
 * 9999` and then run the example `$ bin/run-example org.apache.spark.examples.streaming.JavaSqlNetworkWordCount
 * localhost 9999`
 */
...
public final class JavaSqlNetworkWordCount {
    ...
    public static void main(final String[] args) {
        ...
        words.foreachRDD(new Function2<JavaRDD<String>, Time, Void>() {
            ...
            public Void call(final JavaRDD<String> rdd, final Time time) {
                SQLContext sqlContext = JavaSQLContextSingleton.getInstance(rdd.context());

                // Convert JavaRDD[String] to JavaRDD[bean class] to DataFrame
                JavaRDD<JavaRecord> rowRDD = rdd.map(new Function<String, JavaRecord>() {
                    public JavaRecord call(final String word) {
...
class JavaSQLContextSingleton {
    ...
    static public SQLContext getInstance(final SparkContext sparkContext) {
        if (instance == null) {
